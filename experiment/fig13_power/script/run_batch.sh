#!/bin/bash
#
# Batch Runner for Power Experiment (Figure 12)
#
# This script runs DRAMSim2 simulations for power analysis
# Benchmarks: nab, pr, lbm + mix1 components (imagick, sssp, stream_add, mcf)
#
...
# System config
SYSTEM_INI="${EXPERIMENT_DIR}/system_power.ini"
...
# Trace files to run for Figure 12
# nab, pr, lbm (direct benchmarks)
# + mix1 components: imagick, sssp, stream_add, mcf
TRACES=(
    # Direct benchmarks for Figure 12
    "SPEC/nab.trc:nab"
    "GAP/pr.trc:pr"
    "SPEC/lbm.trc:lbm"
    # mix1 components (imagick, sssp, stream_add, mcf)
...
run_trace() {
    ...
    local hit_rate=$(grep "Row Buffer Hit Rate" "${output_file}" | tail -1 | grep -oP '[\d.]+(?=%)')
    echo "    Power: ${power}W, Hit Rate: ${hit_rate}%"
...
echo "============================================"
echo ""
echo "Next: Run 'python script/plot_power.py' to generate the chart"
