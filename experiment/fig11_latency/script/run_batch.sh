#!/bin/bash
#
# Batch Runner for Latency Profile Experiment (Figure 11)
#
# This script runs DRAMSim2 simulations for all specified traces
# and saves results to trace_log directory
#
# Usage: ./run_batch.sh [config_name]
#   config_name: DRAM, Conv, or SMART (default: all)
...
# System config (local to this experiment)
SYSTEM_INI="${EXPERIMENT_DIR}/system_latency.ini"
...
# Trace files to run (Figure 11 uses only stream_triad)
TRACES=(
    "STREAM/stream_triad.trc:stream_triad"
)

# Number of cycles to simulate (0 = run until trace ends)
# Need enough cycles for transactions to complete and generate latency data
NUM_CYCLES=10000000
...
print_header() {
    ...
    local device_ini="$2"
    echo "============================================"
    echo " Latency Profile Experiment (Fig 11)"
    echo "============================================"
...
run_trace() {
    ...
    # Extract latency info from output
    local avg_latency=$(grep "Average Latency" "${output_file}" | tail -1 | grep -oP '[\d.]+(?=\s*ns)' || echo "N/A")
